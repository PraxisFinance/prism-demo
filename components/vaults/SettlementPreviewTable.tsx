...
export function SettlementPreviewTable({
  vault,
  profile,
}: SettlementPreviewTableProps) {
  const { market } = vault;

  if (profile === "standard") {
    return (
      <p className="text-xs text-muted-foreground">
        Standard deposits earn the vault{"\u2019"}s spot APY directly {"\u2014"}{" "}
        no Prism market exposure.
      </p>
    );
  }
  ...
  const settle =
    profile === "elevated" ? elevatedSettlementApy : stableSettlementApy;

  return (
    <div className="flex flex-col gap-2">
      <span className="font-bold">Expected performance</span>
