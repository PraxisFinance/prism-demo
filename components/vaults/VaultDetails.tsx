 ...
 *     Deposit/Withdraw action panel and the user's live position card(s)
 *     stacked in their own column alongside — see plan/figma-mapping.md.
...
export function VaultDetails({ vault }: VaultDetailsProps) {
  ...
  const depositAmount = useUiStore((state) => state.depositAmount);
  const setDepositAmount = useUiStore((state) => state.setDepositAmount);
