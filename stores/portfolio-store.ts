...
/**
 * Portfolio of Positions. Client-side Zustand store persisted to localStorage
 * (survives refresh / browser close). Cleared on wallet disconnect — see
 * `resetPortfolioOnDisconnect`. See plan/04-mock-data-and-state.md §9.2.
 *
    ...
    subscribeWithSelector((set, get) => ({
      ...
      withdraw: ({ positionId, amountUsd }) => {
        ...
        if (remaining <= 0.01) {
          ...
          set((state) => ({
            positions: state.positions.map((p) =>
              p.id === positionId ? { ...p, principalUsd: remaining } : p
            ),
          }))
        }
...
/** Call from the wallet disconnect flow (e.g. ConnectWalletButton) to clear the portfolio alongside the wallet. */
